...
func TestSelect_Postgres(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sql, args, err := build.Select(tt.q, table, nil, d, 100, 1000)
			if err != nil {
...
func TestSelect_TableWithoutSchema(t *testing.T) {
	...
	sql, _, err := build.Select(query.Query{}, table, nil, dialect.Postgres(), 100, 1000)
	if err != nil {
		t.Fatalf("Select: %v", err)
	}
...
func TestSelect_RejectsInvalidLimits(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, _, err := build.Select(query.Query{}, table, nil, d, tt.defaultLimit, tt.maxLimit)
			if err == nil {
...
func TestSelect_Errors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, _, err := build.Select(tt.q, table, nil, d, 100, 1000)
			if err == nil {
				t.Errorf("Select: expected error, got nil")
			}
		})
